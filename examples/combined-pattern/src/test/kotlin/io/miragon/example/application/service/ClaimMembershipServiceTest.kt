...
class ClaimMembershipServiceTest {
    ...
    fun `should claim membership when a spot is available`() {
        // Given
        val claimedMembership = membership.claim()
        every { processedOperationRepository.existsById(operationId) } returns false
        every { repository.find(membership.id) } returnsMany listOf(membership, claimedMembership)
        ...
        verify(exactly = 1) { repository.save(claimedMembership) }
    ...
    fun `should reject membership when no spot is available`() {
        // Given
        val rejectedMembership = membership.reject()
        every { processedOperationRepository.existsById(operationId) } returns false
        every { repository.find(membership.id) } returnsMany listOf(membership, rejectedMembership)
        ...
        verify(exactly = 1) { repository.save(rejectedMembership) }
    }

    @Test
    fun `should not reserve another spot when operation is already processed`() {
        // Given
