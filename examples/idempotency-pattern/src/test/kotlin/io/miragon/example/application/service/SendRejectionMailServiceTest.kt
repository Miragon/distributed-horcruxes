...
class SendRejectionMailServiceTest {
    ...
    private val underTest = SendRejectionMailService(repository, IdempotentOperationExecutor(processedOperationRepository))
    ...
    private val operationId = OperationId("${membership.id.value}-serviceTask_sendRejectionMail")
    ...
    fun `should load membership and record operation when not processed yet`() {
        ...
        // When
        underTest.sendRejectionMail(membership.id, operationId)
